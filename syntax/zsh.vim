...
" . and : commands
syn match   zshBuiltins         '\%(^\|\s\)[.:]\ze\s'

" Generated by make-commands.zsh
syn keyword zshBuiltins
           \ alias autoload bg bindkey break builtin bye cap cd chdir clone command comparguments compcall 
           \ compctl compdescribe compfiles compgroups compquote comptags comptry compvalues continue cube 
           \ declare dirs disable disown echo echotc echoti emulate enable eval exec exit export false fc fg 
           \ float functions getcap getln getopts hash history integer jobs kill let limit local logout popd 
           \ print printf pushd pushln pwd r read readonly rehash return sched set setcap setopt shift source 
           \ stat suspend test times trap true ttyctl type typeset ulimit umask unalias unfunction unhash 
           \ unlimit unset unsetopt vared wait whence where which zcompile zformat zftp zle zmath_cube zmodload 
           \ zparseopts zprof zpty zregexparse zsocket zstyle ztcp
syn keyword zshContrib
           \ _cdr actionformats bg bg_bold bg_no_bold branchformat catch cdr chpwd_recent_add chpwd_recent_dirs 
           \ chpwd_recent_filehandler color colors colour command complete completer context cursor debug 
           \ disable disown enable fg fg_bold fg_no_bold files flags fned formats g handler hgrevformat histed 
           \ hook_com hooks incarg key list mailcap match max min nslookup nvcsformats p pager parent place pop 
           \ priority prompt prompt_ quiltcommand reporter ret rprompt running s specified stagedstr sum tetris 
           \ tetriscurses throw toggle tty unspecified unstagedstr vcs vcs_info vcs_info_hookadd 
           \ vcs_info_hookdel vcs_info_lastmsg vcs_info_printsys vcs_info_setsys verbose widget x zargs zcalc 
           \ zcp zed zkbd zln zmathfunc zmathfuncdef zmv zrecompile zsh_directory_name_generic
...
hi def link zshBuiltins         Keyword
hi def link zshContrib          Keyword
